...
function sumSubsidy() {
    ...
    document.getElementById('showtotalsubsidy').innerHTML = parseInt(total_subsidy.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')) + ' ' + 'บาท';
    document.getElementById('total_subsidy').innerHTML = '=' + ' ' + parseInt(total_subsidy.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')) + ' ' + 'บาท' ;
    console.log('--------')
